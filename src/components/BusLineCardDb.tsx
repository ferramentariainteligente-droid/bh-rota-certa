...
import { BusLine } from "@/hooks/useBusLinesFromDb";

interface BusLineCardDbProps {
  line: BusLine;
}

export const BusLineCardDb = ({ line }: BusLineCardDbProps) => {
  ...
  const scheduleGroups = groupSchedules(line.schedules);
  const nextDeparture = line.schedules.find(horario => {
    const now = new Date();
      ...
      <CardContent className="p-6">
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t">
          ...
          <div className="text-sm text-muted-foreground">
            {line.schedules.length} horários disponíveis
          </div>
        </div>
      </CardContent>
    </Card>
